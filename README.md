...
```bash
MeteoPipe-Lambdas$ sam build --use-container
```
...
```bash
MeteoPipe-Lambdas$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
MeteoPipe-Lambdas$ sam local start-api
MeteoPipe-Lambdas$ curl http://localhost:3000/
```
...
```bash
MeteoPipe-Lambdas$ sam logs -n HelloWorldFunction --stack-name MeteoPipe-Lambdas --tail
```
...
Tests are defined in the `tests` folder in this project. Use PIP to install
the [pytest](https://docs.pytest.org/en/latest/) and run unit tests.

```bash
MeteoPipe-Lambdas$ pip install pytest pytest-mock --user
MeteoPipe-Lambdas$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name MeteoPipe-Lambdas
```
